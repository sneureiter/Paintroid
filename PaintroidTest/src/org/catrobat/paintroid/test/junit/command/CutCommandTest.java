...
public class CutCommandTest extends CommandTestSetup {
	...
	protected void setUp() throws Exception {
		...
		mStampBitmapUnderTest.eraseColor(Color.TRANSPARENT);
		mCommandUnderTest = new CutCommand(mStampBitmapUnderTest, new Point(mCanvasBitmapUnderTest.getWidth() / 2,
				mCanvasBitmapUnderTest.getHeight() / 2), mCanvasBitmapUnderTest.getWidth(),
				mCanvasBitmapUnderTest.getHeight(), 0);
		mCommandUnderTestNull = new CutCommand(null, null, 0, 0, 0);
	}
	...
	public void testRunRotateStamp() {
		...
		PaintroidAsserts.assertEquals(mStampBitmapUnderTest.getPixel((int) mPointUnderTest.x, (int) mPointUnderTest.y),
				Color.TRANSPARENT);

		mStampBitmapUnderTest.setPixel(0, 0, Color.CYAN);
		PaintroidAsserts.assertEquals(mStampBitmapUnderTest.getPixel(0, 0), Color.CYAN);
